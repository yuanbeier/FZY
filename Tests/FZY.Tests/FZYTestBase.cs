...
namespace FZY.Tests
{
    public abstract class FZYTestBase : AbpIntegratedTestBase<FZYTestModule>
    {
        ...
        protected void UsingDbContext(Action<FZYDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FZYDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FZYDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FZYDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FZYDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FZYDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FZYDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FZYDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FZYDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FZYDbContext, Task<T>> func)
        {
            T result;
